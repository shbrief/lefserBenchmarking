...
LEfSe_iter_tb_nonNA <- getLEfSeIterSummary(LEfSeDatDir, 20)
...
wd <- "~/Projects/lefserBenchmarking/vignettes/gingival/data"
LEfSe_iter_tb_nonNA <- readr::read_csv(file.path(wd, "LEfSe_20iters.csv"))
```

## lefser
```{r}
lefser <- readr::read_csv(file.path(wd, "gingival_a0.01_w0.01_lda3_lefser.csv"))
```
...
lefser_reformat <- lefser %>%
    rename(feature = features, lefser_LDA = scores) %>% 
    mutate(feature = gsub("_", "", feature) %>% gsub("\\.", "", .)) %>%
    select(feature, lefser_LDA)

num_detected <- rowSums(LEfSe_iter_tb_nonNA[2:21] != 0)
lefse_reformat <- LEfSe_iter_tb_nonNA %>%
    mutate(num_detected = num_detected) %>% 
    select(feature, LDA_iter_5, LDA_iter_30, LDA_iter_100, num_detected)
...
ggsave("Figures/SupFig2A_LEfSe_25features.png")
```
...
ggplot(dat2) +
    geom_line(aes(iter, LDA, col = new_names), show.legend = FALSE) +
    ylim(3, 3.6) +
    ...
    labs(title = "Effect of LEfSe's bootstrap on its reproducibility",
         ...
         x = "# of Bootstrap Iterations")
ggsave("Figures/SupFig2B_LEfSe_17features.png")
```
