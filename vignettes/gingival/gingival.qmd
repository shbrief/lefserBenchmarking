...
## Summary table (in percentage) of detected annotation
lefse_taxa <- data.frame(taxa = lefse_tb$most_specific_taxa, tool = "LEfSe")
lefser_taxa <- data.frame(taxa = lefser_tb$most_specific_taxa, tool = "lefser")
mm_taxa <- data.frame(taxa = mm_tb$most_specific_taxa, tool = "MM")

all_taxa <- rbind(lefse_taxa, lefser_taxa, mm_taxa)
all_taxa_tb <- table(all_taxa$tool, all_taxa$taxa, useNA = "ifany") 
all_taxa_tb/rowSums(all_taxa_tb)
```
...
# Plot
## Venn diagrams
Selected features with the strict thresholds
...
ggsave("Figures/SupFig3D_gingival_venn_diagram.png")
```
